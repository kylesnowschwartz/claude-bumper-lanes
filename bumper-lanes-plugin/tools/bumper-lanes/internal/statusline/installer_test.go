...
// TestBumperBinPath verifies path detection works.
func TestBumperBinPath(t *testing.T) {
	path := bumperBinPath()

	// Should return non-empty string
	if path == "" {
		t.Error("bumperBinPath() returned empty string")
	}

	// Should be an absolute path or "bumper-lanes" fallback
	if path != "bumper-lanes" && !filepath.IsAbs(path) {
		t.Errorf("bumperBinPath() = %q, want absolute path or %q", path, "bumper-lanes")
	}
}
